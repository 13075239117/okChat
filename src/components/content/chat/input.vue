    ...
    export default {
        ...
        methods: {
            showView() {
                ...
                this.$store.state.ws3.send(JSON.stringify({
            ...
            send() {
                ...
                this.$store.state.ws2.send(JSON.stringify(this.nowMsg))
            },
            accept() {
        ...
        mounted() {
            ...
            // this.$store.state.ws2 = this.$store.state.ws2
            // this.$store.state.ws2 = new WebSocket(`wss://${location.host}/websocket`)
            // socketClient(this.$store.state.ws2, this.userrr, this.leftMsg, this.allMsg)

            //websocket
            // this.$store.state.ws2.onopen = event => {
            //     this.$store.state.ws2.send(JSON.stringify(this.userrr));
            //     console.log('已连接');
            // }
            this.$store.state.ws2.onmessage = e => {
                console.log('收到新消息');
                var date = new Date().getTime()
                var res = JSON.parse(e.data)
                // if (this.$store.state.recentFri[this.$route.query.id] < date) {
                //更新date
                console.log(54546564);
                // axios({
                //     method: 'post',
                //     url: '/chat/recentDate',
                //     data: {
                ...
                this.$store.state.isread2[res.friId] = 'false'
                // this.$store.commit('isread2',res.data[2])
                console.log('recentMsg', this.$store.state.recentMsgg);
                this.$store.dispatch('recentUpdate')
                // }
                this.newMsg = res.msg
                this.leftMsg.push(res.msg)
                console.log(this.newMsg);
            }
            // this.$store.state.ws2.onclose = function (e) {
            //     // reject("Connection closed.")

            //     console.log('websocket 断开: ' + e.code + ' ' + e.reason + ' ' + e.wasClean)
            //     console.log(e);
            // };
            this.$store.state.ws2.onerror = function (e) {
                console.log(e);
            }
            //分割线
        ...
        updated() {
            // console.log(this.$store.state.ws2);
        ...
        destroyed() {
            console.log('8888');
            // this.$store.state.ws2.close() //离开路由之后断开websocket连接
        },
    }
</script>
<style scoped>
