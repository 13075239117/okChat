        ...
        <v-card class="mx-auto" max-width="344">
            <v-img src="./迪丽热巴.jpg" height="200px">
                <v-card-title class="white--text mt-8">
                    <v-avatar size="56">
                        <img id="me" @click='changeAvatar' alt="user" :src="$store.state.user[0].avatar">
                    </v-avatar>
                    <p class="ml-3">
            ...
            <v-text-field v-model="input" hide-details flat label="个性签名" solo>
                <template v-slot:append>
                    <v-btn class="mx-0" depressed @click='label'>
                        保存
                    </v-btn>
                </template>
            </v-text-field>
                ...
                <v-card color='#1F7087'>
                    <v-card-title class="avatar">
                        <v-avatar color="grey darken-3" size='40'>
                            ...
                            :src="$store.state.user[0].avatar">
                            </v-img>
                        </v-avatar>
                        <span class="title font-weight-light" rows='1'>Joko</span>
                    </v-card-title>
...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            check(item) {
                //  console.log(this.$store.state.commitShow[item.userid][item.date]);
                //  $store.state.commitShow[item.userid][item.date] && $store.state.commitShow[item.userid][item.date] !==undefined
            },
            send() {
