        ...
        <v-list class="list" two-line>
            ...
            <template v-else v-for="(chat,index) in $store.state.recentUpdate">
                ...
                <v-list-item @click='enterMsg(index,chat)'>
                    ...
                    <div v-if='$store.state.isread2[chat.id] == "false"'><span style="color: rgb(48, 116, 129);">未读</span></div>
                </v-list-item>
            </template>
            <!-- <v-divider></v-divider> -->
        </v-list>
...
<script>
    ...
    export default {
        ...
        methods: {
            enterMsg(index, chat) {
                // this.
                this.$store.state.isread2[chat.id] = 'true'
                ...
                axios({
                    ...
                    data:{
                        userid:chat.id, 
                        friid:this.$store.state.user[0].id,
                    }
                })
            }
        },
        ...
        async mounted() {
            ...
            if (Object.keys(this.$store.state.recentFri).length == 0) {
                axios({
                    ...
                    // console.log(this.$store.state.isread2);
